...
Priority order (first one that logs in successfully wins):
  1. Shoonya — if SHOONYA_USER_ID etc. are set
  2. Dhan    — if DHAN_CLIENT_ID + DHAN_ACCESS_TOKEN are set
...
def get_broker():
    ...
    try:
        ...
        if broker.login():
            ...
            return broker
        print("[NEO] Shoonya login failed — trying next broker")
    except Exception as e:
        print(f"[NEO] Shoonya unavailable ({e}) — trying next broker")
    ...
    if client_id and access_token:
        try:
            ...
            if broker.login():
                ...
                return broker
            print("[NEO] Dhan auth failed — falling back to MockBroker")
        except Exception as e:
            print(f"[NEO] Dhan unavailable ({e}) — falling back to MockBroker")
